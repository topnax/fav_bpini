...
class VrpFinderBloc extends Bloc<VrpFinderEvent, VrpFinderState> {
  ...
  List<GyroscopeEvent> gyroEvents = List<GyroscopeEvent>();
  ...
  Stream<VrpFinderState> mapEventToState(
    ...
    if (event is GyroLoaded) {
      c++;
      yield GyroRetrieved(event.gyroEvents, c);
    } else if (event is LoadCamera) {
      gyroscopeEvents.listen((GyroscopeEvent event) {
        debugPrint("event retrieved");
        gyroEvents.add(event);
        dispatch(GyroLoaded(gyroEvents));
      });
